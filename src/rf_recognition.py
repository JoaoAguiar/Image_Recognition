...
def main():
    """
    Main function to measure execution time and run the Random Forest algorithm.
    """
    # Record the start time of the algorithm
    start_time = time.time()

    # Run the Random Forest algorithm
    best_model = run_rf_algorithm()
    ...
    generate_file(test_predictions, "rf_predictions.txt")
...
def run_rf_algorithm():
    """
    Runs the Random Forest Classifier with K-fold cross-validation to tune the model's performance.
    It selects the best model based on precision and generates predictions for test data.
    
    Returns:
        RandomForestClassifier: The best trained Random Forest model.
    """
    ...
    # Perform cross-validation with different values of 'n_estimators' (1 to 50)
    for i in range(1, 51):
        current_precision = 0
