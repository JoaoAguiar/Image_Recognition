...
def main():
    """
    Main function to measure execution time and run the K-Nearest Neighbors algorithm.
    """
    # Record the start time of the algorithm
    start_time = time.time()

    # Run the K-Nearest Neighbors algorithm
    best_model = run_knn_algorithm()
    ...
    generate_file(test_predictions, "knn_predictions.txt")
...
def run_knn_algorithm():
    """
    Runs the K-Nearest Neighbors Classifier with K-fold cross-validation to tune the model's performance.
    It selects the best model based on precision and generates predictions for test data.
    
    Returns:
        KNeighborsClassifier: The best trained K-Nearest Neighbors model.
    """
    ...
    # Perform cross-validation with different values of 'n_neighbors' (1 to 50)
    for k in range(1, 51):
        current_precision = 0
    ...
    print(f"Best model found with n_neighbors={best_k} and precision={best_precision:.4f}")
    return best_model
