...
def main():
    """
    Main function to measure execution time and run the Decision Tree algorithm.
    """
    # Record the start time of the algorithm
    start_time = time.time()

    # Run the Decision Tree algorithm
    best_model = run_decision_tree_algorithm()
    ...
    generate_file(test_predictions, "dt_predictions.txt")
...
def run_decision_tree_algorithm():
    """
    Runs the Decision Tree Classifier with K-fold cross-validation to tune the model's performance.
    It selects the best model based on precision and generates predictions for test data.
    ...
    # Perform cross-validation with different values of 'max_depth' (1 to 50)
    for i in range(1, 51):
        current_precision = 0
    ...
    print(f"Best model found with max_depth={best_k} and precision={best_precision:.4f}")
    return best_model
...
def get_test_data():
    ...
    for _, _, file_names in os.walk("../testing/"):
        for file in file_names:
            ...
            image = cv2.imread(image_path, cv2.IMREAD_COLOR)
            if image is not None:
                test_images.append(image)  # Store the test image
            else:
                print(f"Warning: Image not found at {image_path}")
    return test_images
