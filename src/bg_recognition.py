...
def main():
    """
    Main function to measure execution time and run the Bagging Classifier algorithm.
    """
    # Record the start time of the algorithm
    start_time = time.time()

    # Run the Bagging Classifier algorithm
    best_model = run_bagging_algorithm()
    ...
    generate_file(test_predictions, "bg_predictions.txt")
...
def run_bagging_algorithm():
    """
    Runs the Bagging Classifier with Naive Bayes as the base estimator.
    Performs cross-validation to tune the number of estimators and selects the best model based on precision.
    
    Returns:
        BaggingClassifier: The best trained Bagging Classifier model.
    """
    ...
    for i in range(1, 51):
        current_precision = 0
        model = BaggingClassifier(GaussianNB(), n_estimators=i, random_state=42)
